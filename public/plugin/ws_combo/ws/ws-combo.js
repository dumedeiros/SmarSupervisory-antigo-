...
(function($){
    ...
    cBox=function(a){
        var p={
            border:[],
            padding:[]
        },pv=['Top','Right','Bottom','Left'],i,f=function(s){
            s=parseInt(s);
            return isNaN(s)?0:s;
        };
    ...
    Ws.Dropdown.fn.init=function(p){
        ...
        cc.input.attr('readonly','readonly').css({
            cursor:'default'
        });
    ...
    Ws.Dropdown.fn.show=function(fEnd,byKey){
        ...
        if(!p.flip)
            uin.css('marginTop','-'+b.h+'px').animate({
                marginTop:0
            },d);
        ...
        cc.__mkc();
        cc.div.addClass(cn+'-'+(p.flip?'above':'below')).css({
            zIndex:8094
        });
        cc.ui.css(css);
    ...
    Ws.Combo.fn.render=function(p){
        ...
        while(++j<l) {
            ...
            if(i.s||i.separator)
                ...
                if(i.d||i.disabled){
                    if(!iOS)html+=' onmouseover="Ws.Combo.'+p.id+'.hl('+j+');"';
                }
                ...
                //ALTERADO 
                html+='><span class="wsc-h" <br>'+h+'</br></span></li>';
    ...
    Ws.Combo.fn.oncombo=function(tb){
        ...
        if(!tb._wsc)
        {
            ...
            cc.hc=function(e){
                ...
                cc.img.css({
                    background:im||''
                }).attr('class', 'dropdown-img'+(im?'':' dropdown-img-empty'));			
                if(cc.expanded){
                    tb._wsc=2;
                    cc.hide();
                    ip=cc.input.get(0);
    ...
    Ws.Combo.fn.onresize=function(p){
        var cc = this,cn=p.className,css={
            overflow:'visible',
            overflowY:'',
            height:'25px',
            width:'1px'
        ...
        if(!cc.ui_li){
            ...
            cc.ui.bind('mousewheel', function(e){
                e.preventDefault();
                return false;
            });
        ...
        if($.browser.msie&&$.browser.version<8)
            cc.ui.find('div.'+cn+'-items li.'+cn+'-separator').css({
                width:'1%'
            });
        if(p.combo)$('#'+p.id+'_cb').css({
            width:'1px'
        });
        cc.ui.children().css({
            width:'1%'
        });
        cc.ui.find('.ws-combo-itop').css({
            width:'1px',
            paddingLeft:0
        });
        cc.ui.css({
    ...
    Ws.Combo.fn.onbeforeshow=function(p,byKey){
        ...
        ci.css({
            paddingLeft:(cc.div.width()-cc.btn.width())+'px'
        });
        ...
        if($.browser.msie&&$.browser.version<8){
            ...
            cc.iefr.css({
                width:cc.ui.outerWidth()+'px',
                height:cc.ui.outerHeight()
            });	
        }
        ...
        if(p.combo){
            ...
            tb.css({
                width:(pb.width-b.padding[1]-b.padding[3]-b.border[1]-b.border[3]-pb.padding[1]-pb.padding[3])+'px'
            });
        }
    ...
    Ws.Combo.fn.select=function(i,a){	
        ...
        cc.img.css({
            background:i.im||''
        }).attr('class', 'dropdown-img'+(i.im?'':' dropdown-img-empty'));	
        if(cc.expanded&&!a){
            cc.hide();
            ip.focus();
            if(ip.select)ip.select();
    ...
    //    Ws.Combo.fn.selectByVal=function(v){
    //        var items=this.settings.items,
    //        i=items.length;
    //        alert(i);
    //        while(i--)
    //            if((items[i].v||items[i].value)==v)
    ...
    Ws.Combo.fn.getSelectedIndex=function(){
        return this.selectedIndex;
    }
    
    //    Ws.Combo.fn.getSelectedText=function(){
    //        var txt;
    ...
    //    }
    //        
    //    Ws.Combo.fn.getSelectedValue=function(){
    //            
    //        var idx = this.selectedIndex, txt;
    //            
    //        $(".dropdown-txt").each(function(){
    //            txt = $(this).val();
    //        });
    //            
    //        return (idx == -1) ? txt : this.settings.items[idx].v;
    //    }
    
    Ws.Combo.fn.getSelectedValue=function(){
        var idx = this.selectedIndex;
        var items=this.settings.items;
        if(idx == -1) return this.hidden.val();
        return items[idx].v;
    }
    ...
    Ws.Combo.fn.hideMe=function(a){
        var cc = this;
        cc.div.hide();
        cc.btn.hide();
    }
    
    Ws.Combo.fn.showMe=function(a){
        var cc = this;
        cc.div.show();
        cc.btn.show();
    }
    
    Ws.Combo.fn.clearMe=function(i){
        var cc=this,p=[];
        cc.input.val("");
        cc.set(p);
    };
    
    Ws.Combo.fn.setAux=function(p){
        if(p instanceof Array)
            p={
                items:p
            };
    ...
    Ws.Combo.fn.showMessage=function(m, t){
//        cc = this;
//        var mDiv = $("<span />").text("M").appendTo("body"), mSize = mDiv.width();
//        mDiv.detach();
//    
...
//        };
//        var minSize = cc.input.width();
//        var calcSize = $.map(m, letterSize);
//        var inputSize = 0;
//        console.log(calcSize)
//        $.each(calcSize, function(i, v) {
//            inputSize += v;
//        });
//            
//        cc.input.width( inputSize );
//        cc.input.val(m);
//        
            //len = m.length;
            //len = ((len) * 9);
